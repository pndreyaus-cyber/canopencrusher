...
typedef struct {
    ...
    volatile bool_t syncFlag; // Need to set, when sending synchronous. Like TPDO4
} CO_CANtx_t;
...
#define CO_LOCK_CAN_SEND(CAN_MODULE) noInterrupts() // Lock CAN interrupt. It locks all interrupts. TODO: Lock only CAN interrupts
#define CO_UNLOCK_CAN_SEND(CAN_MODULE) interrupts() // TODO: Unlock only CAN interrupts
