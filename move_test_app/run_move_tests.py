...
class SerialRobotClient:
    ...
    def wait_for_prefix(
        self, prefixes: Iterable[str], timeout_s: float
    ) -> Optional[str]:
...
def run_next_PathPoint(
    ...
    map_reply = client.wait_for_prefix(
        prefixes=["MAP", "MAJ"], timeout_s=move_timeout_s
    )
    ...
    for node_id in range(0, client.get_axes_num()):
        ...
        if abs(target_position - parsed_position) <= position_tolerance_deg:
            ...
            print(
                f"Axis {node_id + 1}: parsed_position={parsed_position}; target_position={target_position}"
            )
            target_reached &= False
...
def generate_path_from_txt(file_path: Path, axes_num: int) -> List[PathPoint]:
    ...
    with open(file_path, "r") as file:
        ...
        while line != "":
            ...
            path.append(
                PathPoint(
                    target_positions_deg=target_positions,
...
def main() -> int:
    ...
    parser.add_argument(
        "--poll-ms", type=int, default=500, help="RPP polling period in ms"
    )
    ...
    for point_num, point in enumerate(path):
        ...
        print(
            f"=== Point {point_num}: {status}; movement time: {(result.result_move_time_ms/1000.0):0.4f}s ===\n"
        )
